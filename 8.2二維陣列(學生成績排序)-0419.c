...
int main()
{
	...
	printf("\n4.列出平均分數及排序平均分數的學生座號及成績--------------------\n");
    for (int i = 0; i < 10; i++){
        student[i][4]=(student[i][1]+student[i][2]+student[i][3])/3;
        printf("第%d位學生,平均分數:%d分\n",student[i][0],student[i][4]);
    }
    ...
    for (int i = 0; i < 9; i++){
        for (int j = 0; j < 9; j++){
            if(student[j][4]<student[j+1][4]){
                ...
                student[j][4] = student[j+1][4];
                student[j+1][0] = temp[0];
                student[j+1][4] = temp[1];
            }
        }
    }
    printf("----------排序平均分數後----------\n");
